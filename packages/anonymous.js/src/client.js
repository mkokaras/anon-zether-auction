...
class Client {
  constructor(web3, zsc, home) {
    ...
    const getEpoch = (timestamp) => {
      return Math.floor(
        (timestamp === undefined ? new Date().getTime() / 1000 : timestamp) /
          epochLength
      );
    };

    const away = () => {
      // returns ms away from next epoch change
      const current = new Date().getTime();
      return (
        Math.ceil(current / (epochLength * 1000)) * (epochLength * 1000) -
        current
      );
    };
    ...
    zsc.events
      .TransferOccurred({}) // i guess this will just filter for "from here on out."
      ...
      .on("data", (event) => {
            ...
            web3.eth.getBlock(blockNumber).then((block) => {
              ...
              web3.eth
                ...
                .then((transaction) => {
                  ...
                  if (account.ignoreTransfers === true) {
                    this.account.ignoreTransfers = false;
                    return;
                  }

                  const value = utils.readBalance(
                    parameters["C"][i],
                    parameters["D"],
                    account.keypair["x"]
                  );
                  ...
                  if (value > 0) {
                    ...
                    console.log(
                      "Transfer of " +
                        ...
                        (account._state.available + account._state.pending) +
                        "."
                    );
                  }
                });
        ...
        if (
          account.keypair["y"].eq(
            bn128.deserialize(event.returnValues["beneficiary"])
          )
        ) {
          ...
          if (secret === undefined) {
            ...
            zsc.methods
              ...
              .on("transactionHash", (hash) => {
                console.log(
                  'Registration submitted (txHash = "' + hash + '").'
                );
              })
              .on("receipt", (receipt) => {
                ...
                console.log("Gas for registering is :" + receipt.gasUsed);
            ...
            zsc.methods
              ...
              .then((result) => {
                ...
                that.account._state.available = utils.readBalance(
                  simulated[0],
                  simulated[1],
                  x
                );
        ...
        zsc.methods
          ...
          .on("receipt", (receipt) => {
            ...
            console.log(
              "Deposit of " +
                value +
                " was successful. Balance now " +
                (account._state.available + account._state.pending) +
                "."
            );

            console.log("Gas for funding account is :" + receipt.gasUsed);
    ...
    this.transferDummy = (C, D, y, u, proof, beneficiaryKey, auc) => {
      const encoded = auc.methods
        .transferEarnings(C, D, y, u, proof, beneficiaryKey)
        .encodeABI();
      ...
      web3.eth.accounts
        .signTransaction(tx, throwaway.privateKey)
        .then((signed) => {
          web3.eth
            ...
            .on("receipt", (receipt) => {
              ...
              console.log("Gas used for transfer is : " + receipt.gasUsed);
              resolve(receipt);
            })
            .on("error", (error) => {
    ...
    this.transfer = (name, value, decoys, beneficiary) => {
      ...
      if (size & (size - 1)) {
        ...
        throw (
          "Anonset's size (including you and the recipient) must be a power of two. Add " +
          (next - size) +
          " or remove " +
          (size - previous) +
          "."
      ...
      return new Promise((resolve, reject) => {
        zsc.methods
          .simulateAccounts(y.map(bn128.serialize), getEpoch())
          .call()
          .then((result) => {
            const deserialized = result.map((account) =>
              ElGamal.deserialize(account)
            );
            if (deserialized.some((account) => account.zero()))
              return reject(
                new Error(
                  "Please make sure all parties (including decoys) are registered."
                )
              ); // todo: better error message, i.e., which friend?
            ...
            const C = y.map((party, i) => {
              const left = ElGamal.base["g"]
                .mul(
                  new BN(
                    i === index[0] ? -value - fee : i === index[1] ? value : 0
                  )
                )
                .add(party.mul(r));
            ...
            const beneficiaryKey =
              beneficiary === undefined ? bn128.zero : friends[beneficiary];

            const encoded = zsc.methods
              .transfer(
              ...
              .then((signed) => {
                web3.eth
                  ...
                  .on("transactionHash", (hash) => {
                    transfers.add(hash);
                    console.log(
                      'Transfer submitted (txHash = "' + hash + '").'
                    );
                  })
                  .on("receipt", (receipt) => {
                    account._state = account._simulate(); // have to freshly call it
                    ...
                    console.log(
                      "Gas used for transfer is : " + receipt.gasUsed
                    );
                    resolve(receipt);
                  })
                  .on("error", (error) => {
    ...
    this.createTransferProof = async (
      name,
      value,
      decoys,
      beneficiary,
      ...
      this.account.ignoreTransfers = ignore_transfer;
      ...
      const result = await zsc.methods
        .simulateAccounts(y.map(bn128.serialize), getEpoch())
        .call();
      ...
      const C = y.map((party, i) => {
        const left = ElGamal.base["g"]
          .mul(
            new BN(i === index[0] ? -value - fee : i === index[1] ? value : 0)
          )
          .add(party.mul(r));
        return new ElGamal(left, D);
      });
      ...
      const u = utils.u(state.lastRollOver + 1, account.keypair["x"]);

      const transferData = {};

      transferData["proof"] = proof.serialize();

      transferData["u"] = bn128.serialize(u);

      transferData["D"] = bn128.serialize(D);

      transferData["y"] = y.map(bn128.serialize);

      transferData["C"] = C.map((ciphertext) =>
        bn128.serialize(ciphertext.left())
      );

      return transferData;
    };

    this.createBurnProof = async (value) => {
      if (this.account.keypair === undefined)
        throw "Client's account is not yet registered!";
      ...
      bidProof["y"] = bn128.serialize(account.keypair["y"]);

      bidProof["proof"] = proof.serialize();
    ...
    this.withdraw = (value) => {
      ...
      if (value > state.available) {
        console.log(
          "Your withdrawal has been queued. Please wait " +
            seconds +
            " second" +
            plural +
            ", for the release of your funds..."
      ...
      if (3100 > wait) {
        // determined empirically. IBFT, block time 1
    ...
    this.lock = (address) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        const [c, s] = utils.signlock(
          address,
          account.keypair,
          utils.gEpoch(state.lastRollOver)
        );
        ...
        zsc.methods
          .lock(
            ...
            c,
            s,
            bn128.serialize(utils.gEpoch(state.lastRollOver))
          )
          .send({ from: home, gas: 6721975 })
          .on("transactionHash", (hash) => {
            console.log('Locking submitted (txHash = "' + hash + '").');
          })
          .on("receipt", (receipt) => {
            console.log("Gas for locking account is :" + receipt.gasUsed);
            resolve(receipt);
          })
          .on("error", (error) => {
            console.log("Locking failed: " + error);
            reject(error);
          });
      });
    };
    ...
    this.unlock = () => {
      ...
      return new Promise((resolve, reject) => {
        zsc.methods
          .unlock(bn128.serialize(account.keypair["y"]))
          .send({ from: home, gas: 6721975 })
          .on("transactionHash", (hash) => {
            console.log('Unlocking submitted (txHash = "' + hash + '").');
          })
          .on("receipt", (receipt) => {
            resolve(receipt);
          })
          .on("error", (error) => {
            console.log("Unlocking failed: " + error);
            reject(error);
          });
      });
    };
    ...
    this.createTransferProofOptimized = async (
      name,
      value,
      decoys,
      beneficiary,
