...
utils.sign = (address, keypair) => {
  ...
  const c = utils.hash(
    ABICoder.encodeParameters(
      ["address", "bytes32[2]", "bytes32[2]"],
      [address, bn128.serialize(keypair["y"]), bn128.serialize(K)]
    )
  );
...
utils.signlock = (address, keypair, epoch) => {
  const k = bn128.randomScalar();
  const K = bn128.curve.g.mul(k);
  const c = utils.hash(
    ABICoder.encodeParameters(
      ["address", "bytes32[2]", "bytes32[2]", "bytes32[2]"],
      [
        address,
        bn128.serialize(keypair["y"]),
        bn128.serialize(K),
        bn128.serialize(epoch),
      ]
    )
  );
