...
class ZetherProof {
  constructor() {
    this.serialize = () => {
      ...
      this.CnG.forEach((CnG_k) => {
        result += bn128.representation(CnG_k.left()).slice(2);
      });
      this.CnG.forEach((CnG_k) => {
        result += bn128.representation(CnG_k.right()).slice(2);
      });
      this.C_0G.forEach((C_0G_k) => {
        result += bn128.representation(C_0G_k.left()).slice(2);
      });
      this.C_0G.forEach((C_0G_k) => {
        result += bn128.representation(C_0G_k.right()).slice(2);
      });
      this.y_0G.forEach((y_0G_k) => {
        result += bn128.representation(y_0G_k.left()).slice(2);
      });
      this.y_0G.forEach((y_0G_k) => {
        result += bn128.representation(y_0G_k.right()).slice(2);
      });
      this.C_XG.forEach((C_XG_k) => {
        result += bn128.representation(C_XG_k.left()).slice(2);
      });
      this.C_XG.forEach((C_XG_k) => {
        result += bn128.representation(C_XG_k.right()).slice(2);
      });
      this.f.getVector().forEach((f_k) => {
        result += bn128.bytes(f_k).slice(2);
      });
  ...
  static prove(statement, witness, fee) {
    ...
    const statementHash = utils.hash(
    ...
    const a = new FieldVector(
      Array.from({ length: 2 * m }).map(bn128.randomScalar)
    );
    const b = new FieldVector(
      (
        new BN(witness["index"][1]).toString(2, m) +
        new BN(index[0]).toString(2, m)
      )
        .split("")
        .reverse()
        .map((i) => new BN(i, 2).toRed(bn128.q))
    );
    const c = a.hadamard(
      b.times(new BN(2).toRed(bn128.q)).negate().plus(new BN(1).toRed(bn128.q))
    );
    const d = a.hadamard(a).negate();
    const e = new FieldVector([
      a.getVector()[0].redMul(a.getVector()[m]),
      a.getVector()[0].redMul(a.getVector()[m]),
    ]);
    const f = new FieldVector([
      a.getVector()[b.getVector()[0].toNumber() * m],
      a.getVector()[b.getVector()[m].toNumber() * m].redNeg(),
    ]);
    ...
    const v = utils.hash(
      ABICoder.encodeParameters(
        ["bytes32", "bytes32[2]", "bytes32[2]", "bytes32[2]", "bytes32[2]"],
        [
    ...
    const recursivePolynomials = (list, a, b) => {
      ...
      const aTop = a.pop();
      const bTop = b.pop();
      const left = new Polynomial([
        aTop.redNeg(),
        new BN(1).toRed(bn128.q).redSub(bTop),
      ]); // X - f_k(X)
    ...
    let P_poly = recursivePolynomials(
      [new Polynomial([new BN(1).toRed(bn128.q)])],
      a.getVector().slice(0, m),
      b.getVector().slice(0, m)
    );
    let Q_poly = recursivePolynomials(
      [new Polynomial([new BN(1).toRed(bn128.q)])],
      a.getVector().slice(m),
      b.getVector().slice(m)
    );
    P_poly = Array.from({ length: m }).map(
      (_, k) => new FieldVector(P_poly.map((P_i) => P_i.coefficients[k]))
    );
    Q_poly = Array.from({ length: m }).map(
      (_, k) => new FieldVector(Q_poly.map((Q_i) => Q_i.coefficients[k]))
    );

    const Phi = Array.from({ length: m }).map(() =>
      ElGamal.commit(key, new BN(0).toRed(bn128.q))
    );
    const Chi = Array.from({ length: m }).map(() =>
      ElGamal.commit(key, new BN(0).toRed(bn128.q))
    );
    const Psi = Array.from({ length: m }).map(() =>
      ElGamal.commit(key, new BN(0).toRed(bn128.q))
    );

    result.CnG = Array.from({ length: m }).map((_, k) =>
      statement["Cn"].multiExponentiate(P_poly[k]).add(Phi[k])
    );
    result.C_0G = Array.from({ length: m }).map((_, k) => {
      const left = new PointVector(statement["C"].map((C_i) => C_i.left()))
        .multiExponentiate(P_poly[k])
        .add(Chi[k].left());
      return new ElGamal(left, Chi[k].right());
    });
    result.y_0G = Array.from({ length: m }).map((_, k) => {
      const left = statement["y"]
        .multiExponentiate(P_poly[k])
        .add(Psi[k].left());
      return new ElGamal(left, Psi[k].right());
    });
    result.C_XG = Array.from({ length: m }).map(() =>
      ElGamal.commit(statement["C"][0].right(), new BN(0).toRed(bn128.q))
    );

    let vPow = new BN(1).toRed(bn128.q);
    for (let i = 0; i < N; i++) {
      // could turn this into a complicated reduce, but...
      const poly = i % 2 ? Q_poly : P_poly; // clunky, i know, etc. etc.
      result.C_XG = result.C_XG.map((C_XG_k, k) =>
        C_XG_k.plus(
          vPow.redMul(
            witness["bTransfer"]
              .redNeg()
              .redSub(new BN(fee).toRed(bn128.q))
              .redMul(
                poly[k].getVector()[
                  (witness["index"][0] + N - (i - (i % 2))) % N
                ]
              )
              .redAdd(
                witness["bTransfer"].redMul(
                  poly[k].getVector()[
                    (witness["index"][1] + N - (i - (i % 2))) % N
                  ]
                )
              )
          )
    ...
    result.tHat = tPolyCoefficients[0]
      .redAdd(tPolyCoefficients[1].redMul(x))
      .redAdd(tPolyCoefficients[2].redMul(x.redPow(new BN(2))));
    const tauX = result.T_1.randomness
      .redMul(x)
      .redAdd(result.T_2.randomness.redMul(x.redPow(new BN(2))));
    ...
    let p = new FieldVector(
      Array.from({ length: N }).map(() => new BN(0).toRed(bn128.q))
    ); // evaluations of poly_0 and poly_1 at w.
    let q = new FieldVector(
      Array.from({ length: N }).map(() => new BN(0).toRed(bn128.q))
    ); // verifier will compute these using f.
    ...
    const DR = statement["C"][0]
      .right()
      .mul(wPow)
      .add(bn128.curve.g.mul(chi.redNeg()));
    const gR = bn128.curve.g.mul(wPow.redSub(psi));
    p = p.add(
      new FieldVector(
        Array.from({ length: N }).map((_, i) =>
          i === index[0] ? wPow : new BN().toRed(bn128.q)
        )
      )
    );
    q = q.add(
      new FieldVector(
        Array.from({ length: N }).map((_, i) =>
          i === index[1] ? wPow : new BN().toRed(bn128.q)
        )
      )
    );
    ...
    for (let i = 0; i < N; i++) {
      ...
      C_XR = C_XR.add(
        new ElGamal(undefined, y_poly.getVector()[Math.floor(i / 2)].mul(vPow))
      );
      if (i > 0) vPow = vPow.redMul(v);
    }
    ...
    const A_b = ElGamal.base["g"]
      .mul(k_b)
      .add(DR.mul(zs[0].redNeg()).add(CnR.right().mul(zs[1])).mul(k_sk));
    const A_X = C_XR.right().mul(k_r); // y_XR.mul(k_r);
    const A_t = ElGamal.base["g"]
      .mul(k_b.redNeg())
      .add(PedersenCommitment.base["h"].mul(k_tau));
    ...
    result.s_b = k_b.redAdd(
      result.c.redMul(
        witness["bTransfer"]
          .redMul(zs[0])
          .redAdd(witness["bDiff"].redMul(zs[1]))
          .redMul(wPow)
      )
    );
    ...
    const o = utils.hash(
      ABICoder.encodeParameters(["bytes32"], [bn128.bytes(result.c)])
    );
    PedersenVectorCommitment.base["h"] =
      PedersenVectorCommitment.base["h"].mul(o);
    PedersenVectorCommitment.base["hs"] = PedersenVectorCommitment.base[
      "hs"
    ].hadamard(ys.invert());
